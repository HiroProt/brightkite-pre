function PersonstreamAssistant() {}

PersonstreamAssistant.prototype = {
  page: 1,
  template: {
  ...
  setup: function() {
    ...
    bk.api.stream('/people/' + bk.object.creator.login + '/objects.json', function(response) {
  ...
  more: function() {
    this.page++;
    bk.api.stream('/people/' + bk.object.creator.login + '/objects.json?page=' + this.page, function(response) {
      var response_object = $j.evalJSON(response)
      $j('#stream')
