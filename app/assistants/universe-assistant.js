function UniverseAssistant() {}

UniverseAssistant.prototype = {
  page: 1,
  template: {
  ...
  setup: function() {
    this.controller.setupWidget(Mojo.Menu.viewMenu, undefined,
      { items: [
        { toggleCmd: 'scene-universe', items: [
    ...
    bk.api.stream('/objects.json', function(response) {
  ...
  more: function() {
    this.page++;
    bk.api.stream('/objects.json?page=' + this.page, function(response) {
      var response_object = $j.evalJSON(response)
      $j('#stream')
