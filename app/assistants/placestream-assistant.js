function PlacestreamAssistant() {}

PlacestreamAssistant.prototype = {
  page: 1,
  template: {
  ...
  setup: function() {
    ...
    bk.api.stream('/places/' + bk.object.place.id + '/objects.json', function(response) {
  ...
  more: function() {
    this.page++;
    bk.api.stream('/places/' + bk.object.place.id + '/objects.json?page=' + this.page, function(response) {
      var response_object = $j.evalJSON(response)
      $j('#stream')
